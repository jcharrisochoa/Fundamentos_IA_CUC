...
class DetectorTapabocas:
    ...
    def __init__(self):
        ...
        self.detecciones = []  # Almacenar detecciones de rostros
        self.face_model = None  # Modelo específico para rostros
    ...
    def init_yolo_model(self):
        ...
        Usa YOLOv8 nano (yolov8n.pt) para detección rápida de personas.
        El modelo detecta la clase 0 (persona) del dataset COCO.
        """
        try:
            # Usar modelo YOLOv8 nano para detección rápida
            # Detectará personas (clase 0 en COCO dataset)
            self.model = YOLO('yolov8n.pt')  # Modelo nano pre-entrenado
            
            # También cargar modelo específico para rostros si está disponible
            try:
                self.face_model = YOLO('yolov8n-face.pt')
                print("Modelo de rostros YOLOv8 cargado correctamente")
            except:
                self.face_model = None
                print("Modelo de rostros no disponible, usando detección de personas")
    ...
    def filter_face_duplicates(self, faces):
        ...
        for face in faces:
            ...
            for accepted in filtered:
                ...
                if iou > 0.3:  # Umbral más bajo para rostros
                    is_duplicate = True
                    break
    ...
    def process_captured_image(self):
        ...
        try:
            ...
            results = self.model(self.imagen_capturada, conf=0.6, verbose=False)
    ...
    def filter_duplicate_detections(self, detections):
        ...
        for det in sorted_detections:
            ...
            if area < 2000:  # Área mínima de 2000 píxeles
                continue
            
            # Validación de proporción (personas no son muy anchas ni muy altas)
            aspect_ratio = width / height if height > 0 else 0
            if aspect_ratio < 0.3 or aspect_ratio > 2.0:  # Proporción razonable para personas
                continue
